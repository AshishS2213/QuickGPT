...
const ChatBox = () => {
  ...
  const {selected, theme, user, axios, token, setUser} = useAppContext()
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if(!user) return toast('Please login to send a message')
        ...
        const {data} = await axios.post(`/api/message/${mode}`, {chatId: selected._id, prompt, isPublished}, {headers: {Authorization: token}})
    ...
    <div className='flex-1 flex flex-col justify-between m-5 md:m-10 xl:mx-30 max-md:mt-14 2xl:pr-40'>
      ...
      <div ref={containerRef} className='flex-1 mb-5 overflow-y-scroll'>
        ...
        <label className='inline-flex items-center gap-2 mb-3 text-sm mx-auto'>
          ...
          />
          {mode !== 'image' && (
            <span className='text-xs text-gray-400 dark:text-gray-300 ml-2'>Switch to Image mode to enable</span>
          )}
        </label>
      {/* Prompt Input Tag */}
